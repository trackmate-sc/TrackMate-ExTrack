...
// So that I can put the TrackMate logging stuff instead of the printing
// to stdout.
//
...
/**
 * methods for minimization of a real-valued function of several variables
...
public class ConjugateDirectionSearch extends MultivariateMinimum implements Cancelable
{
	...
	public static class OptimizationError extends RuntimeException
	{
		...
		public OptimizationError( final String s )
		{
			super( s );
		}
	}
	...
	/**
	 * step is a steplength parameter and should be set equal to the expected
	 * distance from the solution. exceptionally small or large values of step
	 * lead to slower convergence on the first few iterations the default value
	 * for step is 1.0
	 */
	public double step = 1.0;

	/**
	 * scbd is a scaling parameter. 1.0 is the default and indicates no scaling.
	 * if the scales for the different parameters are very different, scbd
	 * should be set to a value of about 10.0.
	 */
	public double scbd = 1.0;

	/**
	 * illc should be set to true if the problem is known to be ill-conditioned.
	 * the default is false. this variable is automatically set, when the
	 * problem is found to to be ill-conditioned during iterations.
	 */
	public boolean illc = false;

	/**
	 * MHL: added this so we can interrupt the optimization if we need to.
	 */
...
//			matprint( "\nEigenvectors of A - ", v );
	...
	private double flin( final double l, final int j )
	{
		if ( j != -1 )
		{ /* linear search */
			for ( int i = 0; i < dim; i++ )
	...
	private void checkBounds( final double[] p )
	{
		for ( int i = 0; i < dim; i++ )
		{
	...
	private void minfit( final int n, double eps, final double tol, final double[][] ab, final double[] q )
	{
		int l = 0, kt, l2, i, j, k;
